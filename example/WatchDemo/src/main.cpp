...
void GPSUpdataTime(uint16_t x, uint16_t y, bool EN)
{
  if (EN == true)
  {
    ...
    {
      rtc.setTime(gps.time.second(), gps.time.minute(), gps.time.hour() + 8, gps.date.day(), gps.date.month(), gps.date.year());
    }
  ...
  //display.fillRect(x, y, 200, 34, GxEPD_WHITE); //clean
  display.setTextColor(GxEPD_BLACK);
  display.setTextSize(2);
